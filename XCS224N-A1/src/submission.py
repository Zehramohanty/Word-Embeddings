...
def compute_co_occurrence_matrix(corpus, window_size=4):
    ...
    for document in corpus:
        ...
        for counter in range(length):
            ...
            for j in range(max(0, counter - window_size), counter):
                target_word_index = word2Ind[document[j]]
                M[target_word_index, ref_word_index] += 1
            for j_2 in range(counter+1, min(length, counter + window_size + 1)):
                target_word_index = word2Ind[document[j_2]]
                M[target_word_index, ref_word_index] += 1
    # ### END CODE HERE ###
